/**
 * Real USDC -> WETH swap via SwapRouter02 (the router the agent has whitelisted).
 * Quotes with QuoterV2 to set amountOutMinimum, then swaps.
 *   set -a; . ./.env; set +a; AMOUNT_USDC=0.2 npx tsx examples/06-swap-v3.ts
 */
...
const QUOTER = "0x61fFE014bA17989E743c5F6cB21bF9697530B21e" as const; // Arbitrum QuoterV2
const amountIn = parseUnits(process.env.AMOUNT_USDC ?? "0.2", 6);
...
// Pick the best fee tier with liquidity.
...
console.log(`Swap ${process.env.AMOUNT_USDC ?? "0.2"} USDC -> WETH @ fee ${best.fee}`);
console.log(`  quoted out: ${formatEther(best.out)} WETH  | min: ${formatEther(amountOutMinimum)} WETH`);

const pre = await client.canSpendToken(USDC, amountIn);
if (!pre.ok) throw new Error(`Blocked: ${pre.reason}`);

const batch = client.buildSwapExactInputV3({
